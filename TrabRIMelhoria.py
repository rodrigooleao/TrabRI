...
def filterHash(query):  # Filtra o hash para palavras que tem na consulta
    ...
    query = [ps.stem(x) for x in query if x not in stopwords]
...
for doc in docs:
    for palavra in doc:
        if (palavra not in stopwords and palavra not in interestedTags and palavra.isalpha()):
            if( not ps.stem(palavra) in hashWords):
                hashWords[ps.stem(palavra)] = [(i , 1)]
            else:
                achou = False
                lista = hashWords[ps.stem(palavra)]
                ...
                if( not achou ):
                    lista.append( (i , 1))
                hashWords[ps.stem(palavra)] = lista
    i+=1

# for elemento in hashWords:
